...
async function registerSlashCommandsForGuild(guild) {
  try {
    const commands = [...slashCommandDefs, ...contextMenuCommandDefs];
    await guild.commands.set(commands);
    console.log(`✅ Sync Commands สำเร็จ: ${guild.name} (${commands.length} คำสั่ง)`);
    console.log('📌 Commands ที่ส่ง Discord:', commands.map(c => c.name).join(', '));
  } catch (e) {
...
client.once(Events.ClientReady, async () => {
  ...
  // Global command cleanup disabled: keep guild command sync as source of truth.
  ...
  console.log('✅ Slash Command Sync เสร็จแล้ว');
  console.log('📌 ใช้ Guild Commands เป็นหลัก: คำสั่งใหม่จะขึ้นทันที');
});
