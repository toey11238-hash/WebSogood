...
const {
  ...
  buildAiReplyPayload, buildHelpEmbed, buildImagineEmbed, buildGiveawayEmbed, buildProphecyEmbed, buildGameHubPayload,
} = panels;

const {
  RPS_EMOJI, RPS_LABEL_TH, rpsRandomChoice, rpsDetermineOutcome,
  parseDiceNotation, rollDice,
  pickEightBallResponse,
  SLOT_SYMBOLS, spinSlots,
  fetchTriviaQuestion,
  matchRpsTrigger, matchSlotsTrigger, matchRollTrigger, matchEightBallTrigger, matchTriviaTrigger,
} = games;
...
// ==========================================
// 🎮 4.55 ระบบมินิเกม/ความบันเทิง (เป่ายิ้งฉุบ, ทอยลูกเต๋า, ลูกแก้ววิเศษ, สล็อต, Trivia)
// ==========================================
const TRIVIA_COOLDOWN_SECONDS = 5; // กันสแปมยิง Open Trivia DB (API ฟรีภายนอก) ถี่เกินไป
const triviaCooldownMap = new Map();
function isOnTriviaCooldown(guildId, userId) {
  const key = `${guildId}:${userId}`;
  const last = triviaCooldownMap.get(key) || 0;
  const now = Date.now();
  if (now - last < TRIVIA_COOLDOWN_SECONDS * 1000) return true;
  triviaCooldownMap.set(key, now);
  return false;
}

// เก็บสถานะคำถาม Trivia ที่กำลังเปิดรอคำตอบอยู่ (messageId -> { correctAnswer, allAnswers, xpReward, answered })
// เก็บในหน่วยความจำเท่านั้น (คำถามมีอายุสั้น ไม่จำเป็นต้องอยู่รอดข้ามการรีสตาร์ทบอทเหมือน Giveaway/Prophecy)
const activeTrivia = new Map();

// ฟังก์ชันสร้าง Payload ของแต่ละเกม แยกออกมาให้ทั้ง Slash Command และคำสั่งพิมพ์แบบไม่มี prefix (ดูหัวข้อ 14.5)
// เรียกใช้ร่วมกันได้ ไม่ต้องเขียนซ้ำ 2 ที่
function buildRpsIntroPayload() {
  const embed = new EmbedBuilder()
    .setColor(0x3498DB)
    .setTitle('✊ เป่ายิ้งฉุบ')
    .setDescription('เลือกไม้ของคุณ! (ใครก็กดเล่นได้ ทุกคนได้ผลลัพธ์ของตัวเอง)');
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('rps_rock').setLabel('🪨 ค้อน').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('rps_paper').setLabel('📄 กระดาษ').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('rps_scissors').setLabel('✂️ กรรไกร').setStyle(ButtonStyle.Secondary)
  );
  return { embeds: [embed], components: [row] };
}

function buildRollResultPayload(parsed) {
  const { rolls, total } = rollDice(parsed.count, parsed.sides);
  const rollsText = rolls.length <= 20 ? rolls.join(', ') : `${rolls.slice(0, 20).join(', ')}, ... (และอีก ${rolls.length - 20} ลูก)`;
  const embed = new EmbedBuilder()
    .setColor(0x3498DB)
    .setTitle(`🎲 ทอย ${parsed.count}d${parsed.sides}`)
    .setDescription(`ผลที่ได้: ${rollsText}\n\n**รวม: ${total}**`);
  return { embeds: [embed] };
}

function buildEightBallPayload(question) {
  const answer = pickEightBallResponse();
  const embed = new EmbedBuilder()
    .setColor(0x2C3E50)
    .setTitle('🎱 ลูกแก้ววิเศษ')
    .addFields(
      { name: '❓ คำถาม', value: question.slice(0, 500) },
      { name: '🔮 คำตอบ', value: answer }
    );
  return { embeds: [embed] };
}

function buildSlotsResultPayload() {
  const { reels, isJackpot, isPair } = spinSlots();
  const resultText = isJackpot ? '💰 **JACKPOT!** โชคดีสุดๆ!' : isPair ? '✨ คู่! เกือบไปแล้ว' : '😅 ลองใหม่นะ';
  const embed = new EmbedBuilder()
    .setColor(isJackpot ? 0xF1C40F : isPair ? 0x3498DB : 0x95A5A6)
    .setTitle('🎰 สล็อตแมชชีน')
    .setDescription(`【 ${reels.join(' | ')} 】\n\n${resultText}`);
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('slots_spin').setLabel('🔄 หมุนอีกครั้ง').setStyle(ButtonStyle.Primary)
  );
  return { payload: { embeds: [embed], components: [row] }, isJackpot };
}

function buildTriviaQuestionPayload(q) {
  const embed = new EmbedBuilder()
    .setColor(0x9B59B6)
    .setTitle('🧠 ตอบคำถามทายปัญหา')
    .setDescription(`**${q.question}**`)
    .addFields(
      { name: '📂 หมวดหมู่', value: q.category || 'ทั่วไป', inline: true },
      { name: '📊 ระดับ', value: q.difficulty, inline: true }
    )
    .setFooter({ text: `ตอบถูกได้ ${q.xpReward} XP โบนัส (ถ้าเปิดระบบเลเวลไว้) — ใครตอบก่อนได้ก่อน!` });
  const row = new ActionRowBuilder().addComponents(
    q.allAnswers.map((ans, i) => new ButtonBuilder().setCustomId(`trivia_${i}`).setLabel(`${String.fromCharCode(65 + i)}. ${ans.slice(0, 70)}`).setStyle(ButtonStyle.Secondary))
  );
  return { embeds: [embed], components: [row] };
}

// เก็บ session ของ Trivia ลง activeTrivia + ตั้งเวลาล้างอัตโนมัติ (ใช้ร่วมกันทั้ง Slash และคำสั่งไม่มี prefix)
function registerTriviaSession(messageId, q) {
  activeTrivia.set(messageId, { correctAnswer: q.correctAnswer, allAnswers: q.allAnswers, xpReward: q.xpReward, answered: false });
  setTimeout(() => activeTrivia.delete(messageId), 5 * 60 * 1000);
}
...
const slashCommandDefs = [
  ...
  new SlashCommandBuilder().setName('gamehub').setDescription('🕹️ โพสต์เมนูเกมแบบกดเล่นได้เลย ไม่ต้องพิมพ์คำสั่ง (แอดมิน/เจ้าของบอท)'),
  new SlashCommandBuilder().setName('rps').setDescription('🎮 เป่ายิ้งฉุบกับบอท'),
  new SlashCommandBuilder()
    .setName('roll')
    .setDescription('🎲 ทอยลูกเต๋า')
    .addStringOption((opt) => opt.setName('เต๋า').setDescription('รูปแบบเช่น d20, 2d6, 4d10 (ไม่ใส่ = 1d6)').setRequired(false)),
  new SlashCommandBuilder()
    .setName('8ball')
    .setDescription('🎱 ถามลูกแก้ววิเศษ')
    .addStringOption((opt) => opt.setName('คำถาม').setDescription('คำถามที่อยากรู้คำตอบ').setRequired(true)),
  new SlashCommandBuilder().setName('slots').setDescription('🎰 หมุนสล็อตแมชชีน'),
  new SlashCommandBuilder()
    .setName('trivia')
    .setDescription('🧠 ตอบคำถามทายปัญหา (ตอบถูกได้ XP โบนัส ถ้าเปิดระบบเลเวลไว้)')
    .addStringOption((opt) => opt.setName('ระดับ').setDescription('ความยาก (ไม่เลือก = สุ่ม)').setRequired(false)
      .addChoices({ name: 'ง่าย', value: 'easy' }, { name: 'ปานกลาง', value: 'medium' }, { name: 'ยาก', value: 'hard' })),
  new SlashCommandBuilder()
    .setName('giveaway')
...
// ==========================================
async function handleLevelUp(guild, defaultChannel, user, cfg, newLevel) {
  let channel = defaultChannel;
  if (cfg.levelingAnnounceChannelId) {
    const announceChannel = await guild.channels.fetch(cfg.levelingAnnounceChannelId).catch(() => null);
    if (announceChannel && announceChannel.isTextBased()) channel = announceChannel;
  }

  const embed = new EmbedBuilder()
    .setColor(0xF1C40F)
    .setDescription(`🎉 ยินดีด้วย ${user} เลื่อนขึ้นเป็น **เลเวล ${newLevel}** แล้ว!`);
  await channel.send({ embeds: [embed] }).catch(() => {});
  ...
  if (roleId) {
    try {
      ...
      if (role && !member.roles.cache.has(roleId)) {
        ...
        await channel.send({ content: `🏅 ${user} ได้รับยศ ${role} จากการถึงเลเวล ${newLevel}!` }).catch(() => {});
      }
    } catch (e) {
...
async function awardLevelXp(message, cfg) {
  ...
  if (after.level > before.level) {
    handleLevelUp(message.guild, message.channel, message.author, cfg, after.level).catch(() => {});
  }
}

// ให้ XP โบนัสจากการเล่นมินิเกม (ถ้าเปิดระบบเลเวลไว้) — ใช้ guild/channel/user ตรงๆ เพราะเกมมาจาก
// Slash Command (interaction) ไม่ใช่ message เหมือน awardLevelXp ด้านบน
async function awardBonusXp(guild, channel, user, cfg, amount) {
  if (!cfg.levelingEnabled || !guild) return;
  const guildId = guild.id;
  const data = getLevelData(guildId);
  const prevEntry = data[user.id] || { xp: 0 };
  const before = calculateLevel(prevEntry.xp || 0);
  const newXp = (prevEntry.xp || 0) + amount;
  const after = calculateLevel(newXp);
  data[user.id] = { xp: newXp, lastMessageAt: Date.now() };
  saveLevelData(guildId);
  if (after.level > before.level) {
    handleLevelUp(guild, channel, user, cfg, after.level).catch(() => {});
  }
}
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // ==========================================
  // 🎮 14.5 คำสั่งเกมแบบไม่ต้องมี prefix (/ หรือ !) — พิมพ์คำเหล่านี้เฉยๆ ก็เล่นได้เลย
  // เพื่อลดความยุ่งยากที่ต้องจำสัญลักษณ์นำหน้าตลอด (ทำงานคู่ขนานกับ /คำสั่ง และ !คำสั่ง แบบเดิม ไม่ได้แทนที่)
  // ออกแบบให้ดักจับเฉพาะรูปแบบที่ชัดเจนพอ (ขึ้นต้นประโยคเป๊ะๆ / มีรูปแบบเฉพาะ) เพื่อไม่ให้ไปชนกับประโยคคุยเล่นปกติ
  // ==========================================
  const rawContent = message.content.trim();

  if (matchRpsTrigger(rawContent)) {
    return message.reply(buildRpsIntroPayload()).catch(() => {});
  }

  if (matchSlotsTrigger(rawContent)) {
    const { payload, isJackpot } = buildSlotsResultPayload();
    await message.reply(payload).catch(() => {});
    if (isJackpot) awardBonusXp(message.guild, message.channel, message.author, cfg, 20).catch(() => {});
    return;
  }

  // "roll" / "ทอย" / "ทอยเต๋า" ต้องอยู่ต้นประโยคและตามด้วยสัญกรณ์เต๋าที่ถูกต้องเท่านั้น (เช่น "roll d20")
  // ถ้าตามด้วยข้อความอื่นที่ไม่ใช่สัญกรณ์เต๋า จะไม่ดักจับ ปล่อยให้เป็นข้อความคุยปกติต่อไป กันชนกับประโยคทั่วไป
  const rollRemainder = matchRollTrigger(rawContent);
  if (rollRemainder !== null) {
    const parsed = parseDiceNotation(rollRemainder);
    if (parsed) {
      return message.reply(buildRollResultPayload(parsed)).catch(() => {});
    }
  }

  // "8ball" / "ลูกแก้ว" ต้องอยู่ต้นประโยคและตามด้วยคำถามอย่างน้อย 1 คำ
  const eightBallQuestion = matchEightBallTrigger(rawContent);
  if (eightBallQuestion !== null) {
    return message.reply(buildEightBallPayload(eightBallQuestion)).catch(() => {});
  }

  const triviaTrigger = matchTriviaTrigger(rawContent);
  if (triviaTrigger) {
    if (isOnTriviaCooldown(guildId, message.author.id)) {
      return message.react('⏳').catch(() => {});
    }
    try {
      const q = await fetchTriviaQuestion(triviaTrigger.difficulty);
      if (!q) return message.reply('❌ ดึงคำถามไม่สำเร็จ ลองใหม่อีกครั้งนะครับ').catch(() => {});
      const sent = await message.reply(buildTriviaQuestionPayload(q));
      registerTriviaSession(sent.id, q);
      return;
    } catch (e) {
      console.error('❌ Trivia (ไม่มี prefix) เกิดข้อผิดพลาด:', e.message);
      return message.reply('❌ ดึงคำถามไม่สำเร็จ (เซิร์ฟเวอร์ Trivia อาจกำลังหน่วง) ลองใหม่อีกครั้งนะครับ 🥲').catch(() => {});
    }
  }
  ...
  if (message.content === '!gamehub') {
    if (!hasPerm) {
      return message.reply({ content: `❌ คุณไม่มีสิทธิ์ใช้คำสั่งนี้! (ID ของคุณ: \`${message.author.id}\`)` }).catch(() => {});
    }
    return message.reply(buildGameHubPayload(message.guild)).catch(() => {});
  }
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'gamehub') {
    if (!checkHasPermission(interaction.user.id, interaction.member, guildId)) {
      return interaction.reply({ content: `❌ คุณไม่มีสิทธิ์ใช้คำสั่งนี้! (ID ของคุณ: \`${interaction.user.id}\`)`, ephemeral: true }).catch(() => {});
    }
    return interaction.reply(buildGameHubPayload(interaction.guild)).catch(() => {});
  }

  if (commandName === 'rps') {
    return interaction.reply(buildRpsIntroPayload()).catch(() => {});
  }

  if (commandName === 'roll') {
    const notation = interaction.options.getString('เต๋า') || '';
    const parsed = parseDiceNotation(notation);
    if (!parsed) {
      return interaction.reply({ content: '❌ รูปแบบเต๋าไม่ถูกต้อง ใช้เช่น `d20`, `2d6`, `4d10` (ลูกเต๋าไม่เกิน 100 ลูก, หน้าไม่เกิน 1000 หน้า)', ephemeral: true }).catch(() => {});
    }
    return interaction.reply(buildRollResultPayload(parsed)).catch(() => {});
  }

  if (commandName === '8ball') {
    const question = interaction.options.getString('คำถาม', true);
    return interaction.reply(buildEightBallPayload(question)).catch(() => {});
  }

  if (commandName === 'slots') {
    const { payload, isJackpot } = buildSlotsResultPayload();
    await interaction.reply(payload).catch(() => {});
    if (isJackpot && interaction.guildId) {
      const gcfg = getGuildConfig(interaction.guildId);
      awardBonusXp(interaction.guild, interaction.channel, interaction.user, gcfg, 20).catch(() => {});
    }
    return;
  }

  if (commandName === 'trivia') {
    if (isOnTriviaCooldown(guildId, interaction.user.id)) {
      return interaction.reply({ content: `⏳ ใจเย็นๆ นะ กันสแปมอยู่ (${TRIVIA_COOLDOWN_SECONDS} วินาทีต่อครั้ง) ลองใหม่อีกสักครู่`, ephemeral: true }).catch(() => {});
    }
    const difficulty = interaction.options.getString('ระดับ') || '';
    await interaction.deferReply();
    try {
      const q = await fetchTriviaQuestion(difficulty);
      if (!q) {
        return interaction.editReply('❌ ดึงคำถามไม่สำเร็จ ลองใหม่อีกครั้งนะครับ').catch(() => {});
      }
      await interaction.editReply(buildTriviaQuestionPayload(q));
      const msg = await interaction.fetchReply();
      registerTriviaSession(msg.id, q);
      return;
    } catch (e) {
      console.error('❌ /trivia เกิดข้อผิดพลาด:', e.message);
      return interaction.editReply('❌ ดึงคำถามไม่สำเร็จ (เซิร์ฟเวอร์ Trivia อาจกำลังหน่วง) ลองใหม่อีกครั้งนะครับ 🥲').catch(() => {});
    }
  }
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    // ---------- ปุ่มเป่ายิ้งฉุบ (ใครก็กดได้ แต่ละคนได้ผลลัพธ์ของตัวเองแบบ ephemeral) ----------
    if (interaction.isButton() && interaction.customId.startsWith('rps_')) {
      const userChoice = interaction.customId.replace('rps_', '');
      const botChoice = rpsRandomChoice();
      const outcome = rpsDetermineOutcome(userChoice, botChoice);
      const outcomeText = outcome === 'win' ? '🎉 คุณชนะ!' : outcome === 'lose' ? '😢 คุณแพ้!' : '🤝 เสมอ!';
      const resultEmbed = new EmbedBuilder()
        .setColor(outcome === 'win' ? 0x2ECA53 : outcome === 'lose' ? 0xE74C3C : 0x95A5A6)
        .setDescription(
          `คุณเลือก ${RPS_EMOJI[userChoice]} ${RPS_LABEL_TH[userChoice]}\n` +
          `บอทเลือก ${RPS_EMOJI[botChoice]} ${RPS_LABEL_TH[botChoice]}\n\n${outcomeText}`
        );
      await interaction.reply({ embeds: [resultEmbed], ephemeral: true }).catch(() => {});
      if (outcome === 'win' && interaction.guildId) {
        const gcfg = getGuildConfig(interaction.guildId);
        awardBonusXp(interaction.guild, interaction.channel, interaction.user, gcfg, 10).catch(() => {});
      }
      return;
    }

    // ---------- ปุ่มหมุนสล็อตอีกครั้ง ----------
    if (interaction.isButton() && interaction.customId === 'slots_spin') {
      const { payload, isJackpot } = buildSlotsResultPayload();
      await interaction.reply({ embeds: payload.embeds, ephemeral: true }).catch(() => {});
      if (isJackpot && interaction.guildId) {
        const gcfg = getGuildConfig(interaction.guildId);
        awardBonusXp(interaction.guild, interaction.channel, interaction.user, gcfg, 20).catch(() => {});
      }
    ...
    // ---------- ปุ่มตอบคำถาม Trivia (คนแรกที่กดคือคนตอบ) ----------
    if (interaction.isButton() && interaction.customId.startsWith('trivia_')) {
      const session = activeTrivia.get(interaction.message.id);
      if (!session) {
        return interaction.reply({ content: '❌ คำถามนี้หมดอายุแล้ว หรือไม่พบข้อมูล', ephemeral: true }).catch(() => {});
      }
      if (session.answered) {
        return interaction.reply({ content: '⏰ มีคนตอบไปแล้ว! ลองข้อต่อไปนะ', ephemeral: true }).catch(() => {});
      }
      const idx = parseInt(interaction.customId.replace('trivia_', ''), 10);
      const chosenAnswer = session.allAnswers[idx];
      const isCorrect = chosenAnswer === session.correctAnswer;
      session.answered = true;
      activeTrivia.delete(interaction.message.id);

      const resultEmbed = new EmbedBuilder()
        .setColor(isCorrect ? 0x2ECA53 : 0xE74C3C)
        .setTitle(isCorrect ? '✅ ตอบถูก!' : '❌ ตอบผิด')
        .setDescription(
          `${interaction.user} เลือก "${chosenAnswer}"\n\n` +
          `เฉลย: **${session.correctAnswer}**${isCorrect ? `\n\n🎉 ได้รับ ${session.xpReward} XP โบนัส!` : ''}`
        );
      await interaction.update({ embeds: [resultEmbed], components: [] }).catch(() => {});

      if (isCorrect && interaction.guildId) {
        const gcfg = getGuildConfig(interaction.guildId);
        awardBonusXp(interaction.guild, interaction.channel, interaction.user, gcfg, session.xpReward).catch(() => {});
      }
      return;
    }

    // ---------- ปุ่มในเมนูเกม (Game Hub) — ใครก็กดได้ ไม่ต้องมีสิทธิ์แอดมิน ----------
    if (interaction.isButton() && interaction.customId === 'gamehub_rps') {
      return interaction.reply({ ...buildRpsIntroPayload(), ephemeral: true }).catch(() => {});
    }

    if (interaction.isButton() && interaction.customId === 'gamehub_slots') {
      const { payload, isJackpot } = buildSlotsResultPayload();
      await interaction.reply({ embeds: payload.embeds, ephemeral: true }).catch(() => {});
      if (isJackpot && interaction.guildId) {
        const gcfg = getGuildConfig(interaction.guildId);
    ...
    if (interaction.isButton() && interaction.customId === 'gamehub_roll') {
      const modal = new ModalBuilder().setCustomId('modal_gamehub_roll').setTitle('🎲 ทอยลูกเต๋า');
      const input = new TextInputBuilder()
        .setCustomId('roll_notation').setLabel('รูปแบบเต๋า เช่น d20, 2d6 (ไม่ใส่ = 1d6)').setStyle(TextInputStyle.Short)
        .setRequired(false).setMaxLength(20);
      modal.addComponents(new ActionRowBuilder().addComponents(input));
      return interaction.showModal(modal).catch(() => {});
    }

    if (interaction.isButton() && interaction.customId === 'gamehub_8ball') {
      const modal = new ModalBuilder().setCustomId('modal_gamehub_8ball').setTitle('🎱 ลูกแก้ววิเศษ');
      const input = new TextInputBuilder()
        .setCustomId('eightball_question').setLabel('คำถามของคุณ').setStyle(TextInputStyle.Short)
        .setRequired(true).setMaxLength(200);
      modal.addComponents(new ActionRowBuilder().addComponents(input));
      return interaction.showModal(modal).catch(() => {});
    }

    if (interaction.isButton() && interaction.customId === 'gamehub_trivia') {
      const hubGuildId = interaction.guildId;
      if (hubGuildId && isOnTriviaCooldown(hubGuildId, interaction.user.id)) {
        return interaction.reply({ content: `⏳ ใจเย็นๆ นะ กันสแปมอยู่ (${TRIVIA_COOLDOWN_SECONDS} วินาที/ครั้ง) ลองใหม่อีกสักครู่`, ephemeral: true }).catch(() => {});
      }
      await interaction.deferReply();
      try {
        ...
        console.error('❌ Trivia (Game Hub) เกิดข้อผิดพลาด:', e.message);
        await interaction.editReply('❌ ดึงคำถามไม่สำเร็จ (เซิร์ฟเวอร์ Trivia อาจกำลังหน่วง) ลองใหม่อีกครั้งนะครับ 🥲').catch(() => {});
      }
      return;
    }

    // ---------- Modal Submit จากเมนูเกม ----------
    if (interaction.isModalSubmit() && interaction.customId === 'modal_gamehub_roll') {
      const notation = interaction.fields.getTextInputValue('roll_notation') || '';
      const parsed = parseDiceNotation(notation);
      if (!parsed) {
        return interaction.reply({ content: '❌ รูปแบบเต๋าไม่ถูกต้อง ใช้เช่น `d20`, `2d6`, `4d10`', ephemeral: true }).catch(() => {});
      }
      return interaction.reply({ ...buildRollResultPayload(parsed), ephemeral: true }).catch(() => {});
    }

    if (interaction.isModalSubmit() && interaction.customId === 'modal_gamehub_8ball') {
      const question = interaction.fields.getTextInputValue('eightball_question').trim();
      return interaction.reply({ ...buildEightBallPayload(question), ephemeral: true }).catch(() => {});
    }
