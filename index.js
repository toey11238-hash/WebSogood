...
const slashCommandDefs = [
  ...
  new SlashCommandBuilder()
    .setName('kick')
    .setDescription('เตะสมาชิกออกจากเซิร์ฟเวอร์ (ต้องมีสิทธิ์ Kick Members)')
  ...
  new SlashCommandBuilder()
    .setName('ban')
    .setDescription('แบนสมาชิกออกจากเซิร์ฟเวอร์ (ต้องมีสิทธิ์ Ban Members)')
  ...
  new SlashCommandBuilder()
    .setName('unban')
    .setDescription('ปลดแบนผู้ใช้ด้วย User ID (ต้องมีสิทธิ์ Ban Members)')
    .addStringOption((opt) => opt.setName('user_id').setDescription('User ID ของผู้ที่ถูกแบน').setRequired(true)),
  new SlashCommandBuilder()
    .setName('timeout')
    .setDescription('Timeout (ปิดปาก) สมาชิกชั่วคราว (ต้องมีสิทธิ์ Moderate Members)')
  ...
  new SlashCommandBuilder()
    .setName('untimeout')
    .setDescription('ยกเลิก Timeout ของสมาชิก (ต้องมีสิทธิ์ Moderate Members)')
    .addUserOption((opt) => opt.setName('ผู้ใช้').setDescription('สมาชิกที่ต้องการยกเลิก Timeout').setRequired(true)),
  new SlashCommandBuilder()
    .setName('warn')
    .setDescription('เตือนสมาชิก (บันทึกประวัติไว้ในระบบ)')
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('ล้างประวัติการเตือนทั้งหมดของสมาชิก (ต้องมีสิทธิ์ Moderate Members)')
    .addUserOption((opt) => opt.setName('ผู้ใช้').setDescription('สมาชิกที่ต้องการล้างประวัติ').setRequired(true)),
  new SlashCommandBuilder()
    .setName('purge')
    .setDescription('ลบข้อความหลายข้อความในห้องนี้พร้อมกัน (ต้องมีสิทธิ์ Manage Messages)')
...
async function handleSlashCommand(interaction) {
  ...
  // ==========================================
  // 🛡️ คำสั่งดูแลเซิร์ฟเวอร์ (Moderation)
  // ==========================================
  if (commandName === 'kick') {
  ...
  if (commandName === 'dream') {
    ...
    if (sub === 'view') {
      ...
      const embed = new EmbedBuilder().setColor(0x9B59B6).setTitle('🌙 ความฝันล่าสุดของเซิร์ฟเวอร์นี้')
  ...
  if (commandName === 'laws') {
    ...
    if (sub === 'book') {
      ...
      const embed = new EmbedBuilder().setColor(0xD4AC0D).setTitle(`📜 ธรรมนูญเซิร์ฟเวอร์ ${interaction.guild.name}`)
  ...
  if (commandName === 'relic') {
    ...
    if (sub === 'inventory') {
      ...
      const embed = new EmbedBuilder().setColor(0x9B59B6).setTitle(`🏺 คลังของวิเศษของ ${targetUser.username}`)
    ...
    if (sub === 'top') {
      ...
      const embed = new EmbedBuilder().setColor(0xF1C40F).setTitle(`🏆 อันดับนักล่าของวิเศษ — ${interaction.guild.name}`)
