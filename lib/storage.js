...
process.on('SIGTERM', async () => {
  try {
    ...
    console.log('📦 ปิด MongoDB connection สำเร็จ');
    process.exit(0);
  } catch (error) {
    console.error(
      '❌ ปิด MongoDB connection ไม่สำเร็จ:',
      error.message
    );
    process.exit(1);
  }
...
const guildConfigSchema = new mongoose.Schema({
  ...
  memoryMaxTokens: { type: Number, default: 3000 }, // ตัดความจำเก่าออกถ้าประเมินโทเคนรวมเกินนี้ (กันส่ง context ยาวเกินจนโดน provider ปฏิเสธ)
  dailyTokenLimit: { type: Number, default: 0 }, // โควต้าโทเคน AI ต่อวันต่อเซิร์ฟเวอร์ (0 = ไม่จำกัด)
  ...
  // ระบบ "ศาลเซิร์ฟเวอร์" (Server Court) — เกมสมมติเพื่อความบันเทิงล้วนๆ ให้สมาชิกฟ้องร้องกันขำๆ แล้ว AI ตัดสิน
  courtEnabled: { type: Boolean, default: false },
  courtCaseCounter: { type: Number, default: 0 },

  // แผง Dashboard สาธารณะ — ข้อความถาวรในห้องที่สมาชิกกดปุ่มดูข้อมูลรวมของทุกระบบได้ ไม่ต้องพิมพ์คำสั่งเยอะๆ
  dashboardChannelId: { type: String, default: '' },
...
// CourtCase Schema (ระบบ "ศาลเซิร์ฟเวอร์" — เกมสมมติเพื่อความบันเทิงล้วนๆ ให้สมาชิกฟ้องร้องกันขำๆ แล้ว AI ตัดสิน)
const courtCaseSchema = new mongoose.Schema({
  ...
  articleRef: { type: Number, default: null }, // อ้างอิงมาตราจากธรรมนูญเซิร์ฟเวอร์ (ถ้ามี)
  verdictText: { type: String, default: '' },
  winner: { type: String, default: 'plaintiff' }, // plaintiff | defendant
  createdAt: { type: Number, default: () => Date.now() },
});
courtCaseSchema.index({ guildId: 1, caseNumber: 1 });
courtCaseSchema.index({ guildId: 1, plaintiffId: 1 });
courtCaseSchema.index({ guildId: 1, defendantId: 1 });
const CourtCaseModel = mongoose.model('CourtCase', courtCaseSchema);

// UserMemory Schema (ระบบจำ "ความสัมพันธ์" กับผู้ใช้แต่ละคน — นิสัย ความสนใจ และอารมณ์/ความรู้สึกที่มีต่อบอท)
const userMemorySchema = new mongoose.Schema({
  guildId: { type: String, required: true },
  userId: { type: String, required: true },
  note: { type: String, default: '' }, // บันทึกสั้นๆ จาก AI เกี่ยวกับนิสัย/ความสนใจ/แนวโน้มน้ำเสียงของผู้ใช้คนนี้
  sentiment: { type: String, default: 'neutral' }, // positive | neutral | negative | hostile
...
// ==========================================
// ⚖️ ระบบ "ศาลเซิร์ฟเวอร์" (Server Court)
// ==========================================
async function createCourtCase(guildId, caseNumber, plaintiffId, defendantId, accusation, articleRef, verdictText, winner) {
  try {
    const doc = await CourtCaseModel.create({
      guildId, caseNumber, plaintiffId, defendantId, accusation,
      articleRef: articleRef || null, verdictText, winner, createdAt: Date.now(),
    });
    return doc.toObject();
  } catch (e) {
    console.error(`⚠️ บันทึกคดีของกิลด์ ${guildId} ไม่สำเร็จ:`, e.message);
    return null;
  }
}

async function getCourtCaseArchive(guildId, limit = 10) {
  try {
    return await CourtCaseModel.find({ guildId }).sort({ caseNumber: -1 }).limit(limit).lean();
  } catch (e) {
    return [];
  }
}

// สถิติคดีของผู้ใช้คนหนึ่ง: ชนะ/แพ้ ทั้งในฐานะโจทก์และจำเลย
async function getCourtRecord(guildId, userId) {
  try {
    const [asPlaintiff, asDefendant] = await Promise.all([
      CourtCaseModel.find({ guildId, plaintiffId: userId }).lean(),
      CourtCaseModel.find({ guildId, defendantId: userId }).lean(),
    ]);
    const plaintiffWins = asPlaintiff.filter((c) => c.winner === 'plaintiff').length;
    const defendantWins = asDefendant.filter((c) => c.winner === 'defendant').length;
    const totalWins = plaintiffWins + defendantWins;
    const totalCases = asPlaintiff.length + asDefendant.length;
    return {
      totalCases,
      totalWins,
      totalLosses: totalCases - totalWins,
    ...
    return { totalCases: 0, totalWins: 0, totalLosses: 0, asPlaintiff: 0, asDefendant: 0, plaintiffWins: 0, defendantWins: 0 };
  }
}

// ==========================================
// 🧠 ระบบจำ "ความสัมพันธ์" กับผู้ใช้แต่ละคน (Per-user Relationship Memory)
// ==========================================
async function getUserMemory(guildId, userId) {
  try {
    return await UserMemoryModel.findOne({ guildId, userId }).lean();
  } catch (e) {
    return null;
  }
}

async function upsertUserMemory(guildId, userId, note, sentiment, interactionCount) {
  try {
    await UserMemoryModel.updateOne(
      { guildId, userId },
      { $set: { note, sentiment, interactionCount, lastUpdatedAt: Date.now() } },
      { upsert: true },
    );
    return true;
  } catch (e) {
    console.error(`⚠️ บันทึกความทรงจำเกี่ยวกับผู้ใช้ ${userId} ไม่สำเร็จ:`, e.message);
    return false;
  }
}

// ขยับตัวนับจำนวนบทสนทนาเฉยๆ (ยังไม่ถึงรอบวิเคราะห์ใหม่ด้วย AI) — เขียนเบาๆ ไม่ต้องเรียก AI ทุกครั้ง
async function bumpUserMemoryCount(guildId, userId, count) {
  try {
    await UserMemoryModel.updateOne(
      { guildId, userId },
      { $set: { interactionCount: count }, $setOnInsert: { note: '', sentiment: 'neutral' } },
      { upsert: true },
    );
    return true;
  } catch (e) {
...
async function clearUserMemory(guildId, userId) {
  try {
    await UserMemoryModel.deleteOne({ guildId, userId });
    return true;
  } catch (e) {
    return false;
  }
