...
function buildHelpEmbed(panelCommand) {
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      `• \`/help\` — แสดงข้อความนี้\n\n` +
      `**🎮 มินิเกม (ตอบถูก/ชนะบางเกมได้ XP โบนัสด้วย ถ้าเปิดระบบเลเวลไว้)**\n` +
      `• \`/gamehub\` — 🕹️ โพสต์เมนูเกมแบบกดเล่นได้เลย ไม่ต้องพิมพ์อะไรอีก (แอดมิน)\n` +
      `• \`/rps\` — เป่ายิ้งฉุบกับบอท\n` +
      `• \`/roll [เต๋า]\` — ทอยลูกเต๋า เช่น \`d20\`, \`2d6\`\n` +
      `• \`/8ball <คำถาม>\` — ถามลูกแก้ววิเศษ\n` +
      `• \`/slots\` — หมุนสล็อตแมชชีน\n` +
      `• \`/trivia [ระดับ]\` — ตอบคำถามทายปัญหาชิง XP\n\n` +
      ...
      `📊 หน้าสถิติอัปเดตสด • 🏆 ระบบเลเวล/XP พร้อมยศรางวัลอัตโนมัติ • 🎨 สร้างภาพด้วย AI ฟรี • 👋 ต้อนรับ/อำลาสมาชิก + ยศอัตโนมัติ • 🎉 จัดกิจกรรมแจกของรางวัล • 🏛️ สภา AI โต้วาทีสด • 🔮 ผนึกคำทำนาย AI เปิดเผยเอง • 🎮 มินิเกม (เป่ายิ้งฉุบ/ทอยเต๋า/สล็อต/Trivia) • 🚫 ระบบ Blacklist/Whitelist • ` +
...
// ==========================================
// 🕹️ เมนูเกม (Game Hub) — แอดมินโพสต์ครั้งเดียว ที่เหลือให้ทุกคนกดเล่นได้เลยไม่ต้องพิมพ์คำสั่งอะไรอีก
// ==========================================
function buildGameHubPayload(guild) {
  const embed = new EmbedBuilder()
    .setColor(0x1ABC9C)
    .setTitle(`🕹️ เมนูเกม — ${guild ? guild.name : ''}`)
    .setDescription(
      'กดปุ่มด้านล่างเพื่อเล่นได้เลย ไม่ต้องพิมพ์คำสั่งอะไรทั้งนั้น!\n\n' +
      '✊ **เป่ายิ้งฉุบ** — ท้าดวลกับบอท\n' +
      '🎰 **สล็อต** — ลองดวงกับสล็อตแมชชีน\n' +
      '🎲 **ทอยเต๋า** — เลือกรูปแบบลูกเต๋าที่ต้องการ\n' +
      '🎱 **ลูกแก้ววิเศษ** — ถามอะไรก็ได้\n' +
      '🧠 **ทายปัญหา** — ตอบถูกได้ XP โบนัส (ถ้าเปิดระบบเลเวลไว้)'
    )
    .setFooter({ text: 'ข้อความนี้ใช้ซ้ำได้เรื่อยๆ ไม่มีวันหมดอายุ' });

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('gamehub_rps').setLabel('เป่ายิ้งฉุบ').setEmoji('✊').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('gamehub_slots').setLabel('สล็อต').setEmoji('🎰').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('gamehub_roll').setLabel('ทอยเต๋า').setEmoji('🎲').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('gamehub_8ball').setLabel('ลูกแก้ววิเศษ').setEmoji('🎱').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('gamehub_trivia').setLabel('ทายปัญหา').setEmoji('🧠').setStyle(ButtonStyle.Primary)
  );
