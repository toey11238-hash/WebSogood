...
function splitHelpText(text, limit = 4000) {
  const pages = [];
  let current = "";
...
function buildHelpEmbed(panelCommand) {
  const fullEmbed = new EmbedBuilder()
    ...
    .setColor(0x9B59B6)
    .setDescription(
      `**คำสั่งทั้งหมดอยู่ใน 4 คำสั่งหลัก: \`/bot\` \`/ai\` \`/server\` \`/community\` (กดแล้วเลือก subcommand ต่อได้เลย)**\n\n` +
      `**คำสั่งทั่วไป**\n` +
      `• พิมพ์คุยกับบอทได้เลยในห้องที่กำหนด (บอทจะจำบทสนทนาไว้คุยต่อเนื่อง และจดจำนิสัย/ความรู้สึกที่มีต่อคุณด้วย)\n` +
      `• \`/ai ask\` — ถาม AI โดยตรง\n` +
      `• \`/ai imagine\` — สร้างภาพด้วย AI (ฟรี) • \`/ai reset\` — ล้างความจำบทสนทนา+ความทรงจำเกี่ยวกับคุณ\n` +
      `• \`/ai memory\` — ดูว่า AI จำอะไรเกี่ยวกับคุณไว้บ้าง (นิสัย/ความสนใจ/ความรู้สึกที่มีต่อคุณ)\n` +
      `• \`/ai usage\` — ดูโควต้าโทเคน AI ที่ใช้ไปวันนี้ • \`/ai limit\` — ตั้งโควต้าโทเคนต่อวัน (แอดมิน)\n` +
      `• \`/community level rank\` หรือ \`!rank\` — ดูเลเวล/XP • \`/community level leaderboard\` หรือ \`!leaderboard\` — ดูอันดับ XP สูงสุด\n` +
      `• \`/community giveaway start\` — เริ่มกิจกรรมแจกของรางวัล (แอดมิน)\n` +
      `• \`/ai council <หัวข้อ>\` — 🏛️ เปิดสภา AI ให้บุคลิกบอท 2 โหมดโต้วาทีกันสดๆ (ฟีเจอร์พิเศษ)\n` +
      `• \`/ai prophecy <ระยะเวลา>\` — 🔮 ให้บอททำนายอนาคตเซิร์ฟเวอร์แบบผนึกเวลา (ฟีเจอร์พิเศษ เพื่อความบันเทิง)\n` +
      `• \`/bot info stats\` — ดูสถิติการใช้งานแบบเรียลไทม์\n` +
      `• \`/bot info help\` — แสดงข้อความนี้\n\n` +
      `**คำสั่งสำหรับเจ้าของบอท/แอดมิน**\n` +
      `• \`${panelCommand}\` หรือ \`/bot info panel\` — เปิดแผงควบคุมบอท (มีหน้าตั้งค่าระบบเลเวลด้วย)\n` +
      `• \`!avatar\` หรือ \`/bot info avatar\` — สุ่มเปลี่ยนรูปโปรไฟล์บอท\n\n` +
      `**🎛️ แผงข้อมูลสมาชิก (Dashboard) — ทางลัดที่ดีที่สุด**\n` +
      `\`/bot dashboard setup\` — ดูตัวอย่างก่อน แล้วเผยแพร่ปุ่มถาวรลงห้องที่เลือก สมาชิกกดดูเลเวล/AI จำอะไรเกี่ยวกับฉัน/ความฝัน/ธรรมนูญ/ของสะสม/สถิติศาล/เปิด Ticket ได้เองโดยไม่ต้องพิมพ์คำสั่งเลย (ปุ่มจะโชว์เฉพาะระบบที่เปิดใช้งานอยู่)\n\n` +
      `**คำสั่งดูแลเซิร์ฟเวอร์ (ต้องมีสิทธิ์ Discord ที่เกี่ยวข้อง) — ทั้งหมดอยู่ใต้ \`/server\`**\n` +
      `• \`/server mod\` — รวมคำสั่งดูแลเซิร์ฟเวอร์ทั้งหมดไว้ที่เดียว: kick, ban, unban, timeout, untimeout, warn, warnings, clearwarnings, purge\n` +
      `• \`/server automod\` — ตั้งค่ากรองคำต้องห้าม/ลิงก์/สแปมเมนชันอัตโนมัติ (ต้องมีสิทธิ์ Manage Server)\n` +
      `• \`/server ticket setup\` — ตั้งค่า + โพสต์ปุ่มเปิด Ticket ติดต่อทีมงาน (ต้องมีสิทธิ์ Manage Server)\n` +
      `• \`/server antiraid\` — ตรวจจับ + ป้องกันการ Raid เซิร์ฟเวอร์อัตโนมัติ (ต้องมีสิทธิ์ Manage Server)\n\n` +
      `**🌙 ฟีเจอร์เฉพาะตัว (ทั้งหมดอยู่ใต้ \`/community\`)**\n` +
      `\`/community dream setup\` — ทุกวันบอทจะนำเรื่องราวที่เกิดขึ้นในเซิร์ฟเวอร์มาแต่งเป็น "ความฝัน" เชิงสัญลักษณ์ ใช้ \`/community dream view\` อ่านความฝันล่าสุด หรือ \`/community dream archive\` ย้อนอ่านคลังเก่าๆ\n` +
      `\`/community laws setup\` — ทุกครั้งที่มีเหตุการณ์ดูแลเซิร์ฟเวอร์ มีโอกาสจุดประกายให้ AI ร่าง "มาตรากฎหมาย" สมมติขำๆ ใหม่ 1 ข้อ ใช้ \`/community laws book\` เปิดอ่านทั้งเล่ม\n` +
      `\`/community relic on\` (ต้องเปิด \`/community dream setup\` ก่อน) — ของวิเศษหลุดจากความฝันทุกคืน ให้แย่งคว้า ของหายากบูสต์ XP x2 ชั่วคราว ใช้ \`/community relic inventory\`/\`top\`/\`gift\`\n` +
      `\`/community court on\` — ยื่นฟ้องกันขำๆ ด้วย \`/community court file\` ให้ AI สวมบทผู้พิพากษาตัดสิน ใช้ \`/community court record\`/\`cases\` ดูสถิติ — เกมสมมติเพื่อความบันเทิงล้วนๆ\n\n` +
