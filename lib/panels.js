...
function buildHelpEmbed(panelCommand) {
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      `• \`/kick\` — เตะสมาชิก • \`/ban\` \`/unban\` — แบน/ปลดแบน\n` +
      `• \`/timeout\` \`/untimeout\` — ปิดปากชั่วคราว/ยกเลิก\n` +
      `• \`/warn\` \`/warnings\` \`/clearwarnings\` — เตือนสมาชิก + ดู/ล้างประวัติ\n` +
      `• \`/purge\` — ลบข้อความหลายอันพร้อมกัน (สูงสุด 100 ข้อความ)\n` +
